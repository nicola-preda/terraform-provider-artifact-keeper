...
| | |
|---|---|
| Validated against | **Artifact Keeper 1.6.3** (2026-07-28) |
| Provider changes needed | none |
| Acceptance suite | green against the live 1.6.3 image |
...
(`internal/provider/provider.go`). This release is `v1.6.3` (AK 1.6.3); consumers pin
`~> 1.6.3`. A provider-only fix that keeps the same validated AK version is rare; if one
...
2. Tag `v<that version>` (e.g. `v1.6.3`) and push it. The `release` workflow runs
...
git -C "$BK" diff v1.6.3 v1.7.0 -- backend/src/api/routes.rs

# 2. Diff each consumed struct. Repeat per row in the map above.
git -C "$BK" diff v1.6.3 v1.7.0 -- backend/src/api/handlers/repositories.rs
git -C "$BK" diff v1.6.3 v1.7.0 -- backend/src/api/handlers/peers.rs
...
Not bugs; scope decisions. Current as of v1.6.x (**37 resources + 3 data sources**).
...
`repository`/`user`/`group`/`permission` and the data sources. Only `peer`,
`migration_source`, and `sso_*` sit outside it; they need external systems (a reachable
peer, a source registry, an IdP) to exercise.
